...
class DataCollectorAndDivider:
    ...
    def extract_frame(self) -> None:
        ...
        for i in range(frame_count):
            ...
            if ret:
                image_path = os.path.join(self.folder_frames_path, f"image_{i}.jpg")
    ...
    def split_dataset(self, train_ratio=0.7, test_ratio=0.15, valid_ratio=0.15):
        ...
        for folder in [TRAIN, TEST, VALID]:
            os.makedirs(os.path.join(self.folder_divided_path, folder), exist_ok=True)

        # Get the list of image files
        image_files = [f for f in os.listdir(self.folder_frames_path) if f.endswith(JPG) or f.endswith(PNG)]
